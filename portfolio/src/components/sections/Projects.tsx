...
export function Projects() {
  ...
  return (
    ...
    >
      ...
      <div className="absolute top-20 left-1/4 w-96 h-96 bg-indigo/10 rounded-full blur-3xl animate-pulse" />
      <div className="absolute bottom-20 right-1/4 w-96 h-96 bg-cyan/10 rounded-full blur-3xl animate-pulse" />

      <div className="max-w-[1440px] mx-auto px-6 md:px-20 relative z-10">
        <div className="text-center mb-20">
          <motion.div
            className="inline-flex items-center gap-2 px-4 py-2 bg-indigo/10 border border-indigo/20 rounded-full mb-6"
            initial={{ opacity: 0, y: 20 }}
            animate={isInView ? { opacity: 1, y: 0 } : {}}
            transition={{ duration: 0.6 }}
          >
            <Star className="text-indigo" size={16} />
        ...
        <div className="grid md:grid-cols-2 gap-8">
          {projects.map((project, index) => (
            <motion.a
              key={project.title}
              href={project.link}
              className="group relative bg-gradient-to-br from-surface to-surface/50 border-2 border-border rounded-3xl overflow-hidden hover:border-indigo/50 transition-all duration-500 hover:scale-[1.03] hover:shadow-2xl hover:shadow-indigo/20"
              initial={{ opacity: 0, y: 30 }}
              animate={isInView ? { opacity: 1, y: 0 } : {}}
              transition={{ duration: 0.6, delay: 0.3 + index * 0.1 }}
            >
              <div className="absolute inset-0 bg-gradient-to-r from-indigo via-purple-500 to-cyan opacity-0 group-hover:opacity-100 transition-opacity duration-500 blur-xl -z-10" />

              <div className="relative h-64 md:h-80 overflow-hidden bg-gradient-to-br from-indigo/20 to-cyan/20">
                <div className="w-full h-full flex items-center justify-center text-6xl font-bold bg-gradient-to-r from-indigo to-cyan bg-clip-text text-transparent">
              ...
              <div className="p-8">
                <h3 className="text-3xl font-bold text-foreground mb-4 group-hover:text-indigo transition-colors">
                  {project.title}
                </h3>
                ...
                <div className="flex flex-wrap gap-2 mb-6">
                  {project.tech.map((tech) => (
                    <span
                      key={tech}
                      className="px-3 py-1.5 bg-surface/50 border border-border text-indigo rounded-lg text-xs font-medium hover:bg-indigo hover:text-white transition-all font-mono"
                    >
                      {tech}
                    </span>
                  ))}
                </div>
                <div className="flex items-center gap-3 text-indigo font-medium group-hover:gap-5 transition-all">
